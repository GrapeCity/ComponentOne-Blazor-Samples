...
namespace DynamicTreeView
{
    public class FileExplorerItemViewCollection : C1SelectDataCollection<FileExplorerItem, FileExplorerItemView>
    {
        ...
        public override FilterExpression? FilterExpression
        {
            get
            {
                return ConvertBackFilterExpression(base.FilterExpression);
            }
        }

        public override bool CanFilter(FilterExpression? filterExpression)
        {
            return base.CanFilter(ConvertFilterExpression(filterExpression));
        }

        public override Task FilterAsync(FilterExpression? filterExpression, CancellationToken cancellationToken = default)
        {
            return base.FilterAsync(ConvertFilterExpression(filterExpression), cancellationToken);
        }

        private FilterExpression? ConvertFilterExpression(FilterExpression expression)
        {
            switch (expression)
            {
                case FilterNotExpression notExpression:
                    return new FilterNotExpression(ConvertFilterExpression(notExpression.NotExpression));
                case FilterNaryExpression naryExpression:
                    return new FilterNaryExpression(naryExpression.FilterCombination, naryExpression.Expressions.Select(ConvertFilterExpression).ToArray());
                case FilterTextExpression textExpression:
                    {
                        var filterPath = textExpression.FilterPath;
                        if (textExpression.FilterPath.StartsWith("Item."))
                            filterPath = textExpression.FilterPath.Substring("Item.".Length);
                        return new FilterTextExpression(filterPath, textExpression.FilterOperation, textExpression.Value, textExpression.MatchCase, textExpression.MatchWholeWord);
                    }
                case FilterOperationExpression operationExpression:
                    {
                        var filterPath = operationExpression.FilterPath;
                        if (operationExpression.FilterPath.StartsWith("Item."))
                            filterPath = operationExpression.FilterPath.Substring("Item.".Length);
                        return new FilterOperationExpression(filterPath, operationExpression.FilterOperation, operationExpression.Value);
                    }
                case FilterPredicateExpression predicateExpression:
                    return new FilterPredicateExpression((list, index) => predicateExpression.Predicate.Invoke(this, index));
            }
            return null;
        }

        private FilterExpression? ConvertBackFilterExpression(FilterExpression expression)
        {
            switch (expression)
            {
                case FilterNotExpression notExpression:
                    return new FilterNotExpression(ConvertBackFilterExpression(notExpression.NotExpression));
                case FilterNaryExpression naryExpression:
                    return new FilterNaryExpression(naryExpression.FilterCombination, naryExpression.Expressions.Select(ConvertBackFilterExpression).ToArray());
                case FilterTextExpression textExpression:
                    {
                        var filterPath = "Item." + textExpression.FilterPath;
                        return new FilterTextExpression(filterPath, textExpression.FilterOperation, textExpression.Value, textExpression.MatchCase, textExpression.MatchWholeWord);
                    }
                case FilterOperationExpression operationExpression:
                    {
                        var filterPath = "Item." + operationExpression.FilterPath;
                        return new FilterOperationExpression(filterPath, operationExpression.FilterOperation, operationExpression.Value);
                    }
                case FilterPredicateExpression predicateExpression:
                    return new FilterPredicateExpression(predicateExpression.Predicate);
            }
            return null;
        }
    }
                ...
                if (_icon == null)
                {
                    try
                    {
                        ...
                        using (MemoryStream memory = new MemoryStream())
                        {
                            ...
                            _icon = $"data:image/png;base64,{Convert.ToBase64String(memory.ToArray())}";
                        }
                    }
                    catch { }
                }
