...
namespace BlazorExplorer
{
    public class StockDataCollection : C1VirtualDataCollection<Stock>
    {
        ...
        {
            string url = $"Stock?skip={startingIndex}&take={count}";
            ...
            var response = await Http.GetFromJsonAsync<StockResponse>(new Uri(url, UriKind.Relative), cancellationToken);
            return new Tuple<int, IReadOnlyList<Stock>>(response.TotalCount, response.Stocks.ToList());
        }
    }
}
