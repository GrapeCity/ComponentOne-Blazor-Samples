...
namespace DataFilterExplorer.Server
{
    public class PriceFilter : ChecklistFilter
    {
        ...
        protected override void OnAttachedStateChanged(AttachedFilterState previous)
        {
            if (AttachedState != AttachedFilterState.Attached)
            {
                return;
            }

            SetPriceIntervals(PriceIntervals);
        }

        private void SetPriceIntervals(List<PriceInterval> intervals)
        {
