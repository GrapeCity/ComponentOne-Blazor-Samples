...
function tryFiles(img, files, path) {
    if (!path || !files) return;

    var _404 = window['#_404'] || ';';
    if (_404.indexOf(img.src + ';') < 0)
        _404 += img.src + ";";
    window['#_404'] = _404;

    var fs = files.split(","),
        src = '';
    while (_404.indexOf(src + ';') >= 0) {
        src = (fs.indexOf("/") < 0 ? path + '/' : '') + fs.pop();
    }
    ...
    if (fs.length == 0) {
        ...
        img.onerror = function () {
            tryFiles(img, fs.join(','), path);
        }
    }
    img.src = src;
}

function imageExists(path, callBack) {
    var img = new Image();
    img.onload = function () {
        callBack(true);
    };
